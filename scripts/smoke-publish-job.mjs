...
import { PublishAdapter } from "../workers/publish/publish-adapter.ts";
...
const pool = new NodePostgresPool({
  ...process.env,
  SERVICE_NAME: "forgestudio-publish-smoke",
  POSTGRES_POOL_MAX: "6",
});
let rabbit;
let kafka;
...
async function createLegacyFixtures() {
  await pool.pool.query(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS publish_smoke_one_active
      ON site_releases("websiteId") WHERE status='ACTIVE';
    CREATE TABLE IF NOT EXISTS custom_code_snippets(
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
      "websiteId" uuid NOT NULL,
      name text,title text,"codeType" text,language text,placement text,location text,
      scope text,"pageId" text,code text,priority integer,conditions jsonb,status text,
      "isEnabled" boolean,"isActive" boolean,"isDraft" boolean
    );
    CREATE TABLE IF NOT EXISTS theme_location_rules(
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
      "websiteId" uuid NOT NULL,"locationType" text,"templateId" text,
      conditions jsonb,"isActive" boolean
    );
  `);
  ...
  await pool.pool.query(
    `INSERT INTO websites(id,"userId",name,slug,status,"editorData","currentRevision")
     VALUES($1,$2,'Publish smoke','publish-smoke','DRAFT',$3::jsonb,1)`,
    [WEBSITE, USER, JSON.stringify({ version: 1, elements: [{ id: "heading", type: "heading", content: "Verified release" }] })],
  );
  await pool.pool.query(
    `INSERT INTO website_revisions(id,"websiteId",revision,"editorData","actorUserId")
     VALUES($1,$2,1,$3::jsonb,$4)`,
    [REVISION_ID, WEBSITE, JSON.stringify({ version: 1, elements: [{ id: "heading", type: "heading", content: "Verified release" }] }), USER],
  );
}
...
async function setupJob() {
  await tenantTransaction(pool, TENANT, async (tx) => {
    ...
    const event = {
      ...
      traceId: TRACE,
      payload: { action: "publish.smoke.requested", websiteId: WEBSITE, sourceRevision: 1 },
    };
    await acceptJob(tx, {
      task,
      event,
      idempotencyKey: "publish-smoke-key-0001",
      payloadRef: `artifact:publish/${WEBSITE}/1`,
      sideEffects: "idempotent",
    });
    await tx.query(
      `INSERT INTO platform.job_resources(
         tenant_id,job_id,resource_type,resource_id,source_revision,requested_by
       ) VALUES($1,$2,'WEBSITE_PUBLISH',$3,1,$4)`,
      [TENANT, JOB, WEBSITE, USER],
    );
  });
}
...
try {
  ...
  const releases = await pool.pool.query(
    `SELECT id,"releaseNumber","sourceRevision",status,payload
     FROM site_releases WHERE "websiteId"=$1 ORDER BY "releaseNumber"`,
    [WEBSITE],
  );
  ...
  console.log("PASS: durable website publish job, outbox dispatch, RabbitMQ worker, immutable release and idempotent replay");
} finally {
