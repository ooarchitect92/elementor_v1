...
import { IntegrationDeliveryAdapter } from "./integration-adapter.ts";

function log(level: "info" | "error", event: string, details: Record<string, unknown> = {}): void {
  const body = JSON.stringify({
    level,
    event,
    service: "forgestudio-integration-worker",
...
async function main(): Promise<void> {
  ...
  const pool = new NodePostgresPool({
    ...process.env,
    SERVICE_NAME: process.env.SERVICE_NAME || "forgestudio-integration-worker",
  });
  ...
  try {
    ...
    worker = await connectJobWorker(
      "integration.deliver",
      new IntegrationDeliveryAdapter(pool),
      pool,
      abort.signal,
      process.env,
    );
