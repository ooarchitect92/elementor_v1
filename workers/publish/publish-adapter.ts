...
import { encryptPrivateForms, splitReleasePayload } from "./release-payload.ts";
...
const PERMANENT = new Set([
  "INVALID_PUBLISH_PAYLOAD_REF","PUBLISH_RESOURCE_MISSING","PUBLISH_RESOURCE_SCOPE_MISMATCH",
  "PUBLISH_SNAPSHOT_INVALID","PUBLISH_SNAPSHOT_HASH_MISMATCH","PUBLISH_RELEASE_CONFLICT",
  "PUBLISH_SUPERSEDED","PUBLISH_PAYLOAD_TOO_COMPLEX","PUBLISH_PUBLIC_PAYLOAD_TOO_LARGE",
  "PUBLISH_PRIVATE_CONFIG_TOO_LARGE","PUBLISH_PRIVATE_CONFIG_INVALID","PUBLISH_DUPLICATE_FORM_ID",
]);
    ...
    try {
      return await tenantTransaction(this.pool, context.tenantId, async (tx) => {
        const resources = await tx.query<{
          resource_id: string; source_revision: number | string; source_hash: string;
          source_payload: unknown; requested_by: string; created_at: string | Date;
        }>(
          `SELECT resource_id,source_revision,source_hash,source_payload,requested_by,created_at
           FROM platform.job_resources WHERE tenant_id=$1 AND job_id=$2 AND resource_type='WEBSITE_PUBLISH' LIMIT 1`,
        ...
        if (!resource.source_payload || typeof resource.source_payload !== "object" || Array.isArray(resource.source_payload)) {
          throw new Error("PUBLISH_SNAPSHOT_INVALID");
        }
        if (releaseContentHash(resource.source_payload) !== resource.source_hash) throw new Error("PUBLISH_SNAPSHOT_HASH_MISMATCH");
        ...
        const { publicPayload, privateForms } = splitReleasePayload(resource.source_payload);
        const contentHash = releaseContentHash(publicPayload);
        const lockedSite = await tx.query<{ id: string }>(`SELECT id FROM websites WHERE id=$1::uuid FOR UPDATE`, [reference.websiteId]);
        if (!lockedSite.rows[0]) throw new Error("PUBLISH_RESOURCE_MISSING");

        const requestKey = `publish-job:${context.jobId}`;
        const requestHash = releaseContentHash({ jobId: context.jobId, sourceRevision: reference.sourceRevision, sourceHash: resource.source_hash });
        ...
        if (existing.rows[0]) {
          ...
          const encrypted = encryptPrivateForms(privateForms, reference.websiteId, release.id);
          await tx.query(
            `INSERT INTO site_release_private(
               "releaseId","websiteId","formDefinitionsCiphertext","formDefinitionsIv","formDefinitionsTag","keyVersion"
             ) VALUES($1::uuid,$2::uuid,$3,$4,$5,$6) ON CONFLICT("releaseId") DO NOTHING`,
            [release.id, reference.websiteId, encrypted.ciphertext, encrypted.iv, encrypted.tag, encrypted.keyVersion],
          );
          return { status: "DELIVERED", receipt: `release:${release.id}:${release.releaseNumber}` };
        }

        const active = await tx.query<{ requestAcceptedAt: string | Date }>(
          `SELECT "requestAcceptedAt" FROM site_releases WHERE "websiteId"=$1::uuid AND status='ACTIVE' LIMIT 1`,
          [reference.websiteId],
        );
        ...
        const sequence = await tx.query<{ next: number | string }>(
          `SELECT COALESCE(MAX("releaseNumber"),0)+1 AS next FROM site_releases WHERE "websiteId"=$1::uuid`, [reference.websiteId],
        );
        ...
        const inserted = await tx.query<{ id: string; releaseNumber: number | string }>(
          `INSERT INTO site_releases(
             "websiteId","releaseNumber","sourceRevision","requestKey","requestHash","contentHash",payload,
             status,"createdBy","requestAcceptedAt","activatedAt"
           ) VALUES($1::uuid,$2,$3,$4,$5,$6,$7::jsonb,'ACTIVE',$8::uuid,$9::timestamptz,clock_timestamp())
           RETURNING id,"releaseNumber"`,
          [reference.websiteId,releaseNumber,reference.sourceRevision,requestKey,requestHash,contentHash,
           JSON.stringify(publicPayload),resource.requested_by,resource.created_at],
        );
