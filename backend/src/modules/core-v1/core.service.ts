...
export async function getEditorState(websiteId: string, userId: string) {
  ...
  const rows = await prisma.$queryRawUnsafe<any[]>(
    `SELECT id,name,slug,status,"editorData","performanceSettings","currentRevision","updatedAt" FROM websites WHERE id=$1::uuid LIMIT 1`,
    websiteId,
  );
...
export async function saveEditorRevision(websiteId: string, userId: string, input: SaveRevisionInput) {
  ...
  const requestHash = contentHash({
    ...
    performanceSettings: input.performanceSettings,
  });
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const rows = await tx.$queryRawUnsafe<any[]>(
      `SELECT "editorData","performanceSettings","currentRevision" FROM websites WHERE id=$1::uuid FOR UPDATE`,
      websiteId,
    );
    ...
    const performanceSettings = access.canEditDesign && input.performanceSettings !== undefined
    ...
    const updated = await tx.$queryRawUnsafe<any[]>(
      `UPDATE websites
       SET "editorData"=$1::jsonb,"performanceSettings"=$2::jsonb,"currentRevision"=$3,"updatedAt"=NOW()
       WHERE id=$4::uuid AND "currentRevision"=$5
       RETURNING "updatedAt"`,
...
export async function getRevision(websiteId: string, userId: string, revision: number) {
  ...
  const rows = await prisma.$queryRawUnsafe<any[]>(
    `SELECT revision,"editorData","performanceSettings","actorUserId","createdAt"
     FROM website_revisions WHERE "websiteId"=$1::uuid AND revision=$2 LIMIT 1`,
    websiteId,
    revision,
  );
...
export async function publishCurrentRevision(websiteId: string, userId: string, requestKey: string) {
  ...
  return prisma.$transaction(async (tx) => {
    const siteRows = await tx.$queryRawUnsafe<any[]>(
      `SELECT id,name,slug,status,"editorData","performanceSettings","currentRevision"
       FROM websites WHERE id=$1::uuid FOR UPDATE`,
      websiteId,
    );
    ...
    const customCode = await tx.$queryRawUnsafe<any[]>(
      `SELECT id,name,title,"codeType",language,placement,location,scope,"pageId",code,priority,conditions,status,"isEnabled","isActive","isDraft"
    ...
    const themeRules = await tx.$queryRawUnsafe<any[]>(
      `SELECT id,"locationType","templateId",conditions,"isActive"
       FROM theme_location_rules WHERE "websiteId"=$1::uuid AND "isActive"=true`,
      websiteId,
    );
    const payload = {
      website: { id: site.id, name: site.name, slug: site.slug, status: "PUBLISHED" },
      editorData: site.editorData,
      performanceSettings: site.performanceSettings,
    ...
    const duplicate = await tx.$queryRawUnsafe<any[]>(
      `SELECT id,"releaseNumber","sourceRevision","contentHash",status,"activatedAt","requestHash"
    ...
    if (duplicate[0]) {
      if (duplicate[0].requestHash !== requestHash) {
        throw new AppError("requestKey was already used with a different release", 409, "IDEMPOTENCY_KEY_REUSED");
      }
    ...
    const release = await tx.$queryRawUnsafe<any[]>(
      `INSERT INTO site_releases
       ("websiteId","releaseNumber","sourceRevision","requestKey","requestHash","contentHash",payload,status,"createdBy","activatedAt")
       VALUES ($1::uuid,$2,$3,$4,$5,$6,$7::jsonb,'ACTIVE',$8::uuid,NOW())
       RETURNING id,"releaseNumber","sourceRevision","contentHash",status,"activatedAt","createdAt"`,
      websiteId,
      releaseNumber,
      sourceRevision,
      requestKey,
...
export async function listReleases(websiteId: string, userId: string) {
  ...
  return prisma.$queryRawUnsafe<any[]>(
    `SELECT id,"releaseNumber","sourceRevision","contentHash",status,"createdBy","createdAt","activatedAt"
     FROM site_releases WHERE "websiteId"=$1::uuid ORDER BY "releaseNumber" DESC LIMIT 100`,
    websiteId,
  );
}
...
export async function activateRelease(websiteId: string, userId: string, releaseId: string) {
  ...
  requirePublisher(String(website.userPermission));
  return prisma.$transaction(async (tx) => {
    // Serialize publish/rollback operations for a website before changing the single ACTIVE release.
    const site = await tx.$queryRawUnsafe<any[]>(`SELECT id FROM websites WHERE id=$1::uuid FOR UPDATE`, websiteId);
    if (!site[0]) throw new AppError("Website not found", 404, "WEBSITE_NOT_FOUND");
    const target = await tx.$queryRawUnsafe<any[]>(
      `SELECT id,"releaseNumber","sourceRevision","contentHash"
       FROM site_releases WHERE id=$1::uuid AND "websiteId"=$2::uuid FOR UPDATE`,
      releaseId,
      websiteId,
    );
    ...
    const activated = await tx.$queryRawUnsafe<any[]>(
      `UPDATE site_releases SET status='ACTIVE',"activatedAt"=NOW() WHERE id=$1::uuid
       RETURNING id,"releaseNumber","sourceRevision","contentHash",status,"activatedAt"`,
      releaseId,
    );
    await tx.$executeRawUnsafe(`UPDATE websites SET status='PUBLISHED',"updatedAt"=NOW() WHERE id=$1::uuid`, websiteId);
    return activated[0];
...
export async function getActivePublishedSite(websiteId: string) {
  const rows = await prisma.$queryRawUnsafe<any[]>(
    `SELECT r.id AS "releaseId",r."releaseNumber",r."sourceRevision",r."contentHash",r."activatedAt",r.payload
     FROM site_releases r WHERE r."websiteId"=$1::uuid AND r.status='ACTIVE' LIMIT 1`,
    websiteId,
  );
